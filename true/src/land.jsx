...
// Add this ABOVE `const LandingPage = () => { ... }`

const CustomerSupportChatbot = ({ features }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState([
    {
      sender: "bot",
      text: "Hi! 👋 I’m the TrueTalk Assistant. I can help you with features, AI & Voice, HD meetings, group meetings, near jobs, secure payments, enterprise, login and getting started free. Ask me anything!"
    }
  ]);
  ...
  const suggestions = [
    "Tell me about HD meetings",
    "What AI & Voice features do you have?",
    "How do payments work?",
    "Show me near jobs & careers",
    "What is TrueTalk for enterprise?",
    "How do I login or get started free?"
  ];

  const getFeatureByTitle = (keyword) => {
  ...
  const buildReply = (text) => {
    ...
    // LOGIN / GET STARTED
    if (q.includes("login") || q.includes("log in") || q.includes("sign in") || q.includes("register") || q.includes("sign up") || q.includes("get started") || q.includes("start free")) {
      return (
        "You can log in or get started free from the top-right of the page. " +
        "Click **Log In** if you already have an account, or **Get Started Free** to start a free trial (no credit card required, 14-day trial, setup in ~5 minutes)."
      );
    }

    // HD MEETINGS
    if (q.includes("hd meeting") || q.includes("video") || q.includes("conference") || q.includes("4k")) {
      const hd = getFeatureByTitle("HD Meetings");
      return (
        (hd ? `**HD Meetings** – ${hd.description}\n\n` : "") +
        "- 4K Ultra HD video with noise cancellation\n" +
        "- Smart screen sharing & virtual backgrounds\n" +
        "- Cloud recording & searchable transcripts\n" +
        "- Breakout rooms and enterprise-grade security"
      );
    }

    // GROUP MEETINGS
    if (q.includes("group meeting") || q.includes("team meeting") || q.includes("group call")) {
      const gm = getFeatureByTitle("Group Meetings");
      return (
        (gm ? `**Group Meetings** – ${gm.description}\n\n` : "") +
        "- Up to 500 participants\n" +
        "- Breakout rooms with AI auto-grouping\n" +
        "- Live polls, quizzes & reactions\n" +
        "- Shared whiteboard, real-time notes and AI meeting assistant"
      );
    }

    // AI & VOICE / AI INTEGRATION / AI VOICE ASSISTANT
    if (
      q.includes("ai") ||
      q.includes("voice") ||
      ...
      const aiIntegration = getFeatureByTitle("AI Integration");
      const aiVoice = getFeatureByTitle("AI Voice Assistant");

      let reply = "**AI & Voice**\n\n";

      if (aiIntegration) {
        reply += `**AI Integration** – ${aiIntegration.description}\n` +
          "- Smart automation & workflow optimization\n" +
          "- Predictive analytics & auto summaries\n" +
          "- Sentiment analysis & custom AI models\n\n";
      }

      if (aiVoice) {
        reply += `**AI Voice Assistant** – ${aiVoice.description}\n` +
          "- Voice commands & hands-free operations\n" +
          "- Real-time voice translation (50+ languages)\n" +
          "- Emotion-aware responses & meeting summaries";
      }
    ...
    if (q.includes("payment") || q.includes("billing") || q.includes("upi") || q.includes("invoice") || q.includes("subscription")) {
      const sp = getFeatureByTitle("Secure Payments");
      return (
        (sp ? `**Secure Payments** – ${sp.description}\n\n` : "") +
        "- Instant payment gateway inside chats\n" +
        "- One-click UPI, cards & netbanking\n" +
        "- Recurring subscriptions & automatic invoices\n" +
        "- Real-time transaction dashboard\n" +
        "- AI fraud detection & encrypted transactions"
      );
    }

    // JOBS / NEAR JOBS
    if (q.includes("job") || q.includes("career") || q.includes("hiring") || q.includes("near jobs") || q.includes("jobs near")) {
      const nj = getFeatureByTitle("Near Jobs");
      return (
        (nj ? `**Near Jobs** – ${nj.description}\n\n` : "") +
        "- Local job and talent matching\n" +
        "- Personalized job recommendations\n" +
        "- Skill-based matching & career analytics\n" +
        "- One-click applications & interview scheduling\n" +
        "- Great for both candidates and companies hiring"
      );
    }

    // ENTERPRISE
    if (q.includes("enterprise") || q.includes("business") || q.includes("company") || q.includes("team")) {
      return (
        "**Enterprise & Teams**\n\n" +
        "- HD meetings and group collaboration for distributed teams\n" +
        "- AI automation across meetings, chat and workflows\n" +
        "- Built-in secure payments for services & subscriptions\n" +
        "- Job & talent network for hiring and internal mobility\n" +
        "- 99.9% uptime SLA and enterprise-grade security"
      );
    }

    // FEATURES (generic)
    if (q.includes("feature") || q.includes("features") || q.includes("what can you do")) {
      return (
        "TrueTalk brings everything into one platform:\n\n" +
        "- HD meetings & large group calls\n" +
        "- AI integration & AI voice assistant\n" +
        "- Near jobs and talent network\n" +
        "- Secure payments inside chats\n" +
    ...
    // FALLBACK
    return (
      "Nice question! 😊 I can help with:\n" +
      "- Features, AI & Voice, HD / Group meetings\n" +
      "- Payments & secure transactions\n" +
      "- Near jobs and hiring\n" +
      "- Enterprise usage, login & getting started free\n\n" +
      "Try asking: *“Tell me about HD meetings”* or *“How do payments work?”*"
    );
  };

  const handleSend = (valueFromSuggestion) => {
    const text = (valueFromSuggestion ?? input).trim();
    if (!text) return;

    const userMessage = { sender: "user", text };
    const botMessage = { sender: "bot", text: buildReply(text) };
    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {!isOpen && (
        <motion.button
          ...
          className="flex items-center justify-center w-14 h-14 rounded-full shadow-xl bg-gradient-to-r from-blue-600 to-purple-600 text-white text-2xl hover:scale-105 active:scale-95 transition-transform"
          whileHover={{ y: -2 }}
        >
          💬
        </motion.button>
      ...
      {isOpen && (
        <motion.div
          ...
          className="w-80 sm:w-96 h-[480px] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden"
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-3 flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold">TrueTalk Assistant</div>
              <div className="text-xs text-blue-100">
                Online · Helping with features, AI, jobs & payments
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              className="w-7 h-7 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-sm"
            >
              ✕
            </button>
          </div>
          ...
          {/* Messages */}
          <div className="flex-1 bg-gray-50 px-3 py-3 space-y-3 overflow-y-auto">
          ...
          {/* Quick suggestions */}
          <div className="px-3 pb-2 space-x-2 flex flex-wrap gap-2">
            {suggestions.map((s, i) => (
              <button
                key={i}
          ...
          <div className="border-t border-gray-200 px-3 py-2 flex items-center gap-2 bg-white">
            <input
              className="flex-1 text-sm px-3 py-2 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400"
              placeholder="Ask about features, AI, jobs, payments..."
              value={input}
...
const LandingPage = () => {
    ...
    const features = [
        ...
        },
        {
            ...
            image: "https://mahadmanpower.com/wp-content/uploads/2024/03/Top-20-Jobs-Near-Me-Hiring-Immediately-1.jpg",
        ...
        description: "Host large group meetings with smart moderation tools, breakout rooms, live reactions, polls, recording, and AI-generated meeting summaries.",
        image: "https://assets-global.website-files.com/63062129119620a44791a2eb/639b7845e9be869771e540b8_mural-blog-images.jpg",
        details: [
            "Up to 500 participants",
        ...
        icon: "👥",
        stats: "100K+ group calls hosted monthly"
    },
    {
        title: "Secure Payments",
        description: "Built-in payment gateway for sending and receiving money inside chats — perfect for businesses, freelancers, and service providers.",
        ...
        icon: "💳",
        stats: "Over $50M processed securely"
    }
    ];
    ...
    <div className="hidden md:flex space-x-8">
        {["Features", "AI & Voice", "Payments", "Jobs", "Enterprise"].map((item, index) => {
            const link =
                item === "Enterprise"
            ...
            <section id="groupmeetings" className="py-20 px-6 bg-gradient-to-br from-purple-50 to-blue-50 relative overflow-hidden">
            <div className="max-w-7xl mx-auto">
                <motion.div
                    className="text-center mb-16"
                    ...
                    transition={{ duration: 0.8 }}
        >
                <div className="inline-flex px-4 py-2 bg-purple-100 text-purple-600 rounded-full text-sm font-medium mb-4">
                    👥 Group Meetings
                </div>
                <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                    Collaborate With Your Team <span className="text-purple-600">Effortlessly</span>
                </h2>
                <p className="text-lg text-gray-600 max-w-3xl mx-auto">
                    Host large team meetings, breakout rooms, whiteboard collaboration, and real-time group chat.
                </p>
        </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                >
                    <div className="rounded-3xl overflow-hidden shadow-2xl">
                        <img
                            src="https://images.unsplash.com/photo-1584697964403-55ceb31f52e9?auto=format&fit=crop&w=1000&q=80"
                            alt="Group Meeting"
            ...
            >
                <ul className="space-y-6 text-gray-700">
                    {[
                        ...
                        "Breakout rooms with AI auto-grouping",
                        "Live polls, quizzes & reactions",
                        "Shared whiteboard & real-time notes",
                        ...
                        >
                            <div className="w-6 h-6 bg-purple-200 rounded-full flex items-center justify-center text-purple-600">
                                ✓
                            </div>
                ...
                <motion.button
                    className="mt-8 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-8 py-4 rounded-xl font-semibold transition-all duration-300 hover:scale-105 shadow-lg"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                >
...
{/* ================= PAYMENTS SECTION ================= */}
<section id="payments" className="py-20 px-6 bg-gradient-to-br from-blue-50 to-indigo-100 relative overflow-hidden">
    <div className="max-w-7xl mx-auto">
        ...
        <motion.div
            ...
            transition={{ duration: 0.8 }}
        >
            <div className="inline-flex px-4 py-2 bg-blue-100 text-blue-600 rounded-full text-sm font-medium mb-4">
                💳 Payments
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
                Smart & Secure <span className="text-blue-600">Payments System</span>
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
                Accept payments, send invoices, track transactions, and manage billing inside TrueTalk.
            </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            >
                ...
                <motion.button
                    className="mt-8 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-8 py-4 rounded-xl font-semibold transition-all duration-300 shadow-lg"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                >
                    Enable Payments
            ...
            <section id="jobs" ref={jobsRef} className="py-20 px-6 bg-gradient-to-br from-gray-50 to-blue-50 relative overflow-hidden">
                <div className="max-w-7xl mx-auto">
                    ...
                    <section id="enterprise-testimonials"></section>
                    <motion.div
            ...
            <CustomerSupportChatbot features={features} />
